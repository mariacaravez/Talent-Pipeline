...
exports.findStudents = (req, res) => {
    ...
    stModel.find(student, req.query.textValue, req.query.optionsValue)
	.then((results)=> {
      //console.log(results);
      res.send(results);
    }).catch((err) => {
...
exports.newstudent = (req, res) => {
	...
	const student_obj = new StudentObj({
	...
	stModel.create(student_obj, (err, data) => {
		if (err)
		    res.status(500).send({message: err.message || "error occured while registering student."});
		else
			res.send(data);
	});
};
...
exports.findJob = (req, res) => {
	jobModel.find(req.query.jodID, req.query.jobdesc, req.query.optionValue)
	.then ((results) => {
		res.send(results);
	}) .catch((err) => {
