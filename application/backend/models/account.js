...
UserAccount.create = (useracct, result) => {
  ...
  db.promise()
    .query("INSERT INTO userAccounts SET ?", useracct)
    .then(([result, fields]) => {
...
UserAccount.verify = (useracct, result) => {
  console.log("ACCOUNT MODEL - user account obj: ", useracct);
  ...
  // NOBODY TOUCH THIS CODE:
    db.query("SELECT userID, email from user WHERE username = ? AND password = ?", [useracct.username, useracct.password], (err, res) => {
      if (err) {
        ...
        console.log("ACCT - Logging res[0]: ", res[0]);
        result(null, res[0]);
        return;
      }
    }
...
UserAccount.resetpass = (useracct, results) => {
  db.query(
    "UPDATE userAccounts SET password = ? WHERE username = ? AND userAccID = ?",
    [useracct.password, useracct.username, useracct.userID],
    (err, res) => {
      if (err) {
        console.log(err);
        results(err, null);
